"""Quickly inspect DDPG results from the `/tmp/openai-*` directories.
"""
import matplotlib
matplotlib.use('Agg')
...
# other, may need to adjust
MAX_POSSIBLE_EPISODE_SCORE = 6.0
...
def plot_monitor(args):
    ...
    for env_idx,mon in enumerate(monitors):
        ...
        # Just in case monitors are off; adjust if we change reward thresholds
        assert np.max(scores) <= MAX_POSSIBLE_EPISODE_SCORE, np.max(scores)
...
def plot_progress(args):
    ...
    environments. Actually let's just have another plot where we include all?
    """
    col_to_skip = [
        'total/epochs',
        'total/duration',
        'train/param_noise_distance',
        'total/steps_per_second',
        'rollout/episodes',           # we should change so it's episodes within epochs
        'rollout/return',             # this is the avg of ALL returns in the expl env
        'rollout/actions_mean',       # we can look at the actions plot
        'rollout/actions_std',        # we can look at the actions plot
        'reference_Q_mean',
        'reference_Q_std',
    ...
    print("skipping these {} columns: {}".format(len(col_to_skip), col_to_skip))
    ...
    # Now repeat for all stats in a second plot.
    nrows, ncols = df.shape[1], 1
    fig, ax = plt.subplots(nrows, ncols, squeeze=False, sharey='row', figsize=(13*ncols,4*nrows))
    title = args.title
    k= 10
    row = 0
    for column in df:
        ...
        row += 1
    plt.tight_layout()
    figname = '{}_progress_ALL_STATS.png'.format(title)
    plt.savefig(figname)
    print("Just saved: {}\n".format(figname))
...
def plot_actions(args):
    ...
    for a_file in act_files:
        with open(join(act_dir,a_file), 'rb') as fh:
            data = np.asarray(pickle.load(fh)) # in case I saved lists by mistake
            # See PPO2 runner.py bottom method for swapping axes.
            if len(data.shape) == 3:
                S = data.shape
                data = data.swapaxes(0,1).reshape(S[0]*S[1], *S[2:])
    ...
    # TODO need to programmatically determine if lenght/angle or deltas.
    action_names = ['x', 'y', 'deltax', 'deltay']
