...
class DiagGaussianPdType(PdType):
    ...
    def pdfromlatent(self, latent_vector, init_scale=1.0, init_bias=0.0,
                     return_logstd=False):
        """Daniel: returns both the density and the mean vector.

        During training, we don't actually use the mean vector; we use the
        density with its sample() method. I added `return_logstd` so we can
        query it from elsewhere in the code more easily. Otherwise, since
        logstd is integrated with the mean vector, doing a naive sess.run on
        the variable returns an error.
        """
        mean = _matching_fc(latent_vector, 'pi', self.size, init_scale=init_scale,
                            init_bias=init_bias)
...
class DiagGaussianPd(Pd):
    """Daniel: what we usually use for continuous control.
    Note the self.std which exponentiates the logstd.
    """
...
def make_pdtype(ac_space):
    """Daniel: see https://github.com/openai/baselines/issues/810
    There is currently no support for tuple action spaces.
    """
